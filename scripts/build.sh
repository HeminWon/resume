...
mapfile -t themes < <(node -e "const fs=require('fs');const path=require('path');const file=path.resolve(process.cwd(),'src/themes/theme-ids.json');const data=JSON.parse(fs.readFileSync(file,'utf8'));if(!Array.isArray(data)){throw new Error('theme-ids.json must be a JSON array');}for(const item of data){if(typeof item==='string'&&item.trim()){console.log(item.trim());}}")
if [[ "${#themes[@]}" -eq 0 ]]; then
  echo "[build] no themes found in src/themes/theme-ids.json" >&2
  exit 1
fi
langs=(zh en)
footer_args=()
